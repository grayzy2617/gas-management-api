...
## DỰ ÁN HỆ THỐNG QUẢN LÝ GAS PRO (PHIÊN BẢN CHÍNH THỨC HOÀN CHỈNH - 4.0)

> **Người thực hiện:** Senior IT Mentor & Developer  
> **Phiên bản:** v4.0 Complete Master System Schema (Phủ trọn vẹn Phase 1 + Phase 2 + Phase 3 + Phase 4)  
> **Ngày lập:** 05/08/2026  
> **Phạm vi (Scope):** 
...
> - **Phase 4 (FINAL):** Ticket Yêu cầu Bảo hành & Bắt buộc Minh chứng Media (`warranty_tickets`), Đề xuất & Phê duyệt Báo giá Linh kiện 3 Bên (`warranty_parts_proposals`), Danh mục Bếp dùng tạm (`temporary_stoves`) & Nhật ký Lịch sử Mượn / Thu hồi Bếp (`temporary_stove_loans`).

---

## 📐 1. TỔNG QUAN VÀ NGUYÊN TẮC THIẾT KẾ TOÀN HỆ THỐNG (3NF)

Cơ sở dữ liệu hoàn chỉnh bao gồm **30 bảng chuẩn hóa 3NF** đáp ứng 100% các yêu cầu nghiệp vụ trong tài liệu BRD v8, User Stories v5 và Prototype UI:
...
3. **Quản lý Bếp Cho Mượn Dùng Tạm (Temporary Stove Loan - WR-005):** Bảng `temporary_stoves` quản lý trạng thái thiết bị (`AVAILABLE` / `BORROWED`), kết hợp bảng `temporary_stove_loans` theo dõi lịch sử mượn và ngày thu hồi giữa Tài xế và Khách hàng.
...
## 🗂️ 3. DANH SÁCH CHI TIẾT CÁC BẢNG (FULL 30 TABLES)
...
### 🟤 PHẦN VII: BẢO HÀNH, SỬA CHỮA & BẾP MƯỢN (PHASE 4 NEW)

#### 27. Bảng `warranty_tickets` (Ticket Yêu cầu Bảo hành & Minh chứng Media)
* **Mô tả:** Quản lý yêu cầu bảo hành bếp gas/thiết bị từ khách hàng, bắt buộc có ảnh/video minh chứng thực tế (WR-001).
...
| `fault_description` | `TEXT` | `NOT NULL` | Mô tả chi tiết hiện tượng lỗi (Bắt buộc) |
| `media_proof_url` | `VARCHAR(255)` | `NOT NULL` | Đường dẫn tệp Ảnh/Video <10s minh chứng (WR-001 Bắt buộc) |
| `status` | `VARCHAR(30)` | `NOT NULL`, `DEFAULT 'PENDING'` | Trạng thái ticket (`PENDING`, `ASSIGNED`, `IN_PROGRESS`, `WAITING_PARTS_APPROVAL`, `COMPLETED`, `CANCELLED`) |
...
#### 28. Bảng `warranty_parts_proposals` (Đề xuất & Phê duyệt Báo giá Linh kiện 3 Bên)
* **Mô tả:** Quản lý chi tiết từng linh kiện thay thế tại thực địa và quy trình phê duyệt 3 bên: Tài xế ➔ Operator ➔ Khách hàng (WR-004).
...
| `driver_id` | `BIGINT` | `FK -> users(id) ON DELETE RESTRICT`, `NOT NULL` | Tài xế lập đề xuất thay linh kiện |
| `product_id` | `BIGINT` | `FK -> products(id) ON DELETE RESTRICT`, `NOT NULL` | Mã linh kiện thay (IC, Van gas, Dây dẫn) |
| `quantity` | `INT` | `NOT NULL`, `DEFAULT 1` | Số lượng linh kiện |
| `unit_price` | `DECIMAL(15,2)` | `NOT NULL` | Đơn giá linh kiện niêm yết (VNĐ) |
| `subtotal` | `DECIMAL(15,2)` | `NOT NULL` | Thành tiền (`quantity * unit_price`) |
| `operator_approval_status`| `VARCHAR(20)`| `NOT NULL`, `DEFAULT 'PENDING'` | Trạng thái Tổng đài duyệt (`PENDING`, `APPROVED`, `REJECTED`) |
...
| `customer_approval_status`| `VARCHAR(20)`| `NOT NULL`, `DEFAULT 'PENDING'` | Trạng thái Khách hàng duyệt (`PENDING`, `APPROVED`, `REJECTED`) |
...
#### 29. Bảng `temporary_stoves` (Danh mục Bếp Cho Mượn Dùng Tạm)
* **Mô tả:** Quản lý tài sản bếp gas dùng tạm của đại lý trang bị cho khách mượn khi sửa bếp chính dài ngày (WR-005).

| Tên trường | Kiểu dữ liệu | Ràng buộc | Ý nghĩa |
|---|---|---|---|
| `id` | `BIGINT` | `PK`, `AUTO_INCREMENT`, `NOT NULL` | Khóa chính bếp mượn |
| `stove_code` | `VARCHAR(50)` | `UNIQUE`, `NOT NULL` | Mã bếp mượn (`BEP-TMP-001`) |
| `model_name` | `VARCHAR(100)` | `NOT NULL` | Tên dòng bếp cho mượn (`Rinnai RV-150`, `Namilux NA-300`) |
| `status` | `VARCHAR(20)` | `NOT NULL`, `DEFAULT 'AVAILABLE'` | Trạng thái bếp (`AVAILABLE`: Rảnh, `BORROWED`: Đang cho mượn, `MAINTENANCE`: Đang bảo trì) |
| `created_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP` | Ngày tạo |
| `updated_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP ON UPDATE` | Ngày cập nhật |

#### 30. Bảng `temporary_stove_loans` (Nhật ký Lịch sử Bàn giao & Thu hồi Bếp Mượn)
* **Mô tả:** Ghi vết thời điểm tài xế bàn giao bếp mượn cho khách và ngày thu hồi bếp quay về kho đại lý (WR-005).
...
| `ticket_id` | `BIGINT` | `FK -> warranty_tickets(id) ON DELETE RESTRICT`, `NOT NULL` | Nối với ticket bảo hành bếp chính |
| `customer_id` | `BIGINT` | `FK -> customers(id) ON DELETE RESTRICT`, `NOT NULL` | Khách hàng mượn bếp |
| `driver_id` | `BIGINT` | `FK -> users(id) ON DELETE RESTRICT`, `NOT NULL` | Tài xế thực hiện bàn giao |
| `borrowed_at` | `TIMESTAMP` | `DEFAULT CURRENT_TIMESTAMP`, `NOT NULL` | Ngày giờ bàn giao cho khách |
| `returned_at` | `TIMESTAMP` | `NULL` | Ngày giờ thu hồi bếp mượn |
| `loan_status` | `VARCHAR(20)` | `NOT NULL`, `DEFAULT 'ACTIVE'` | Trạng thái mượn (`ACTIVE`: Đang mượn, `RETURNED`: Đã thu hồi) |
| `notes` | `TEXT` | `NULL` | Ghi chú tình trạng bếp thu hồi |

---

## 🔗 4. BẢNG MÔ TẢ QUAN HỆ PHASE 4 BỔ SUNG (RELATIONSHIP MATRIX PHASE 4)

| Bảng Nguồn | Bảng Đích | Loại quan hệ | Khóa ngoại (Foreign Key) | Hành vi xóa | Diễn giải Nghiệp vụ |
|---|---|---|---|---|---|
| `customers` | `warranty_tickets` | 1 - N | `warranty_tickets.customer_id` | `RESTRICT` | 1 Khách hàng tạo nhiều ticket bảo hành. |
| `products` | `warranty_tickets` | 1 - N | `warranty_tickets.product_id` | `RESTRICT` | 1 Sản phẩm/Bếp gas phát sinh bảo hành. |
| `users` (Driver) | `warranty_tickets` | 1 - N | `warranty_tickets.driver_id` | `SET NULL` | 1 Tài xế phụ trách sửa nhiều ticket. |
| `warranty_tickets` | `warranty_parts_proposals` | 1 - N | `warranty_parts_proposals.ticket_id` | `CASCADE` | 1 Ticket bảo hành có N dòng đề xuất thay linh kiện. |
| `products` (Part) | `warranty_parts_proposals` | 1 - N | `warranty_parts_proposals.product_id` | `RESTRICT` | Linh kiện thay thế (IC, Van, Dây gas). |
| `temporary_stoves` | `temporary_stove_loans` | 1 - N | `temporary_stove_loans.temporary_stove_id` | `RESTRICT` | 1 Bếp dùng tạm có lịch sử N lượt mượn. |
| `warranty_tickets` | `temporary_stove_loans` | 1 - N | `temporary_stove_loans.ticket_id` | `RESTRICT` | Lượt mượn bếp gắn liền với ticket bảo hành bếp chính. |

---

## 🎯 5. KẾT LUẬN TOÀN BỘ CƠ SỞ DỮ LIỆU HOÀN CHỈNH MASTER V4.0
1. **Phủ trọn vẹn 30 bảng chuẩn 3NF**, đồng bộ 100% với tài liệu nghiệp vụ BRD v8, User Stories v5 và Prototype UI.
2. Đáp ứng chuẩn Enterprise toàn diện: Auth RBAC, Nhập kho Multi-item gối đầu 30 ngày, Khách sỉ/lẻ mua nợ B2B/B2C, Sổ cái ví lương Ledger tài xế, Báo sự cố hỏng xe thực địa, Quy trình phê duyệt linh kiện bảo hành 3 bên và Quản lý bếp dùng tạm.
